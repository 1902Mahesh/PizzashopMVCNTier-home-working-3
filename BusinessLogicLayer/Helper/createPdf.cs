...
{
    public byte[] CreatePdf(OrderDetailsViewModel model)
    {
        ...
        using (MemoryStream ms = new MemoryStream())
        {
            using (PdfWriter writer = new PdfWriter(ms))
            {
                using (PdfDocument pdf = new PdfDocument(writer))
                {
                    ...
                    leftTable.AddCell(new Cell().Add(new Paragraph($"Name: {model.CustomerDetails?.Customername}"))
                        .SetBorder(Border.NO_BORDER).SetPadding(0).SetMargin(0));
                    leftTable.AddCell(new Cell().Add(new Paragraph($"Mob: {model.CustomerDetails?.Phone}"))
                        .SetBorder(Border.NO_BORDER).SetPadding(0).SetMargin(0));
                    ...
                    rightTable.AddCell(new Cell().Add(new Paragraph($"Date: {model.PlacedOn}"))
                        .SetBorder(Border.NO_BORDER).SetPadding(0).SetMargin(0));
                    rightTable.AddCell(new Cell().Add(new Paragraph($"Section: {model?.SectionName}"))
                        .SetBorder(Border.NO_BORDER).SetPadding(0).SetMargin(0));
                    rightTable.AddCell(new Cell().Add(new Paragraph($"Table: {string.Join(", ", model.TableDetails!)}"))
                        .SetBorder(Border.NO_BORDER).SetPadding(0).SetMargin(0));
                    ...
                    foreach (var tax in model.Taxes ?? new List<TaxListViewModel>())
                    {
                        var taxCal = tax.TaxType ? (tax.TaxValue * model.SubTotal)/100 : tax.TaxValue;
                        totalAmount += taxCal;
                        leftTable.AddCell(CreateCellNew(tax.TaxName));
                        rightTable.AddCell(CreateCellNew($"₹{taxCal:F2}"));
                    }
                    ...
                    leftTable.AddCell(CreateCellNew("Total Amount Due: ", true, isBlue: true, customBlue: customBlue));
                    rightTable.AddCell(CreateCellNew($"₹{model.SubTotal + totalAmount:F2}", true, isBlue: true, customBlue: customBlue));
                    ...
                    paymentCell.Add(CreateCellNew("Payment Information: ", true, isBlue: true, customBlue: customBlue));
                    ...
                    table.AddCell(CreateCellNew("Thank you!", isBlue: true, customBlue: customBlue, bold: true)
                        .SetTextAlignment(TextAlignment.CENTER));
